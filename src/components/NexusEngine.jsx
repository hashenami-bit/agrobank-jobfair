...
export default function NexusEngine() {
  ...
  const targetColorRef = useRef(hexToRgb01(slide.accent));
  const currentColorRef = useRef(hexToRgb01(slide.accent));

  // Update target color whenever the slide accent changes; render loop lerps to it.
  useEffect(() => {
    targetColorRef.current = hexToRgb01(slide.accent);
  }, [slide.accent]);
  ...
  useEffect(() => {
    ...
    const fsSource = `#version 300 es
      ...
      void main() {
          // Core color comes from JS; edges fade to a darker version of the same hue.
          vec3 coreColor = u_color;
          vec3 edgeColor = u_color * 0.45;

          float edgeFactor = abs((v_instance / u_instances) * 2.0 - 1.0);
          vec3 finalColor = mix(coreColor, edgeColor, pow(edgeFactor, 2.0));
    ...
    const render = (time) => {
      ...
      // Lerp current color toward the slide's target accent — ~0.08 per frame
      // reaches target in ~0.6s at 60fps. Snap when within 0.002 to stop rewriting
      // the uniform every frame from floating-point drift.
      ...
      if (Math.abs(dr) > 0.002 || Math.abs(dg) > 0.002 || Math.abs(db) > 0.002) {
        ...
        c[1] += dg * lerp;
        c[2] += db * lerp;
      } else if (c[0] !== t[0] || c[1] !== t[1] || c[2] !== t[2]) {
        c[0] = t[0];
        c[1] = t[1];
        c[2] = t[2];
      }
      gl.uniform3f(u_color, c[0], c[1], c[2]);
